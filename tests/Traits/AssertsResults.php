...
trait AssertsResults
{
    ...
    public function assertResultsContain(
        ...
        $results->pluck($key)->each(function ($result) use ($expected) {
            $this->assertTrue($expected->contains($result), "Expected result missing: $result");
        });
    }
    ...
    public function assertResultsDontContain(
        ...
        $unexpected->pluck($key)->each(function ($unexpected) use ($results) {
            $this->assertFalse(
    ...
    public function assertResultsCount(Collection $results, Collection $expected): void
    {
